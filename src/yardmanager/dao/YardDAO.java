...
public class YardDAO {
	...
	public void addUser(String employee, String username, String pass, String clearance) {
		
		try {
			con.createStatement().executeUpdate("INSERT INTO Users (Employee, Username, Password, Clearance) VALUES ('"+employee+"', '"+username+"', '"+pass+"', '"+clearance+"')");
	...
	public String[][] userData() {
		...
		try {
			...
			rs = con.createStatement().executeQuery("SELECT * FROM Users");
			while(rs.next()) {
				data[i][0] = rs.getString("Employee");
				data[i][1] = rs.getString("Username");
				data[i][2] = rs.getString("Password");
				data[i][3] = rs.getString("Clearance");
				i+=1;
			}
	...
	public void deleteUser(String Username) {
		try {
			con.createStatement().executeUpdate("DELETE FROM Users WHERE Username='"+Username+"'");
	...
	public String[] containerData(String container) {
		String[] data = new String[8];
		data[0] = container;
		data[1] = "bad";
		try {
			rs = con.createStatement().executeQuery("SELECT ISOcode, Mass, Seal, DOM, Acceptance, CustomerCode, CustomerName FROM Interchanges WHERE Container='"+container+ "' AND Status='still in'");
			if(!rs.next()) return data;
			data[1] = rs.getString("ISOcode");
			data[2] = rs.getString("Mass");
			data[3] = rs.getString("Seal");
			data[4] = rs.getString("DOM");
			data[5] = rs.getString("Acceptance");
			data[6] = rs.getString("CustomerCode");
			data[7]	= rs.getString("CustomerName");
		} catch (SQLException e) {}finally {
		    try { rs.close(); } catch (Exception e) {}
		    try { con.close(); } catch (Exception e) {}
		}
	...
	public int[][] getContainers(int lvl) {
		size = 0;
		int i = 0;
		
		try{
			rs = con.createStatement().executeQuery("SELECT X FROM Inventory WHERE Levels <="+lvl);
			while(rs.next()) size++;
		} catch(SQLException e) {}
		
			int[][] containers = new int[size][6];
		try{
			rs = con.createStatement().executeQuery("SELECT X, Y, Length, Width, Levels, Coverage FROM Inventory WHERE Levels <="+lvl);
			while(rs.next()) {
				containers[i][0] = rs.getInt("X");
				containers[i][1] = rs.getInt("Y");
				containers[i][2] = rs.getInt("Length");
				containers[i][3] = rs.getInt("Width");
				containers[i][4] = rs.getInt("Levels");
				containers[i][5] = rs.getInt("Coverage");
				i+=1;
			}
		} catch(SQLException e) {} finally{
			try { rs.close(); } catch (Exception e) {}
